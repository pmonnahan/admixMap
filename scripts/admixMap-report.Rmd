...
eigval = read.table(params$eigenval)
pdat = read.table(params$phenodat)
...
```{r case-numbers-sub}
fam %>% mutate(phenotype = V6) %>% group_by(V6) %>% summarize(n=n()) %>% knitr::kable()
```
...
eigval %>% mutate(id = row_number()) %>% ggplot(aes(x = id, y = V1)) + geom_bar(stat="identity") + ylab("Variance Proportion") + xlab("PC")
```
...
for (i in 1:as.numeric(yaml$gwas$pc_num)){
  for (j in (i+1):as.numeric(yaml$gwas$pc_num)){
    xvar = paste0("PC",i)
    yvar = paste0("PC",j)
    pdat %>% ggplot(aes_string(x=xvar,y=yvar,color='sex',shape='pheno')) + geom_point()
  }
}
```
