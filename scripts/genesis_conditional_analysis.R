...
option_list <- list( 
  ...
  make_option(c("-F", "--minimum_frequency"), default = "0.1",
              help="Minimum frequency of marker for conducting conditional analysis.  Low frequency covariates will result in errors in convergence of null model fit"),
...
mdat = read.table(opt$marker, header = T)
...
Freq = as.numeric(opt$minimum_frequency)

Covars = c(paste0("PC", 1:as.numeric(opt$pc_number)), 'A')
...
mdat %<>% mutate(scanID = IID) %>% select(scanID, A)
# Check frequency of marker
freq = mdat %>% filter(!is.na(A)) %>% select(A) %.>% sum(.$A, na.rm=T) / nrow(.)
if (freq < Freq){
  write.table(tibble(), outName, quote=F, row.names = F)
  print(paste0("Marker frequency is ", freq))
  print("Marker selected for use as covariate in conditional analysis is at too low of a frequency to proceed.  Errors in convergence of null model fit are likely to result.")
} else {
  ...
  pc.df %<>% left_join(mdat, by = "scanID") 
  ...
  if (opt$distribution_family == "binomial"){
    nullmod <- fitNullModel(scanAnnot, outcome = "pheno", covars = Covars, cov.mat = myGRM, family = binomial, max.iter = 10000)
  } else{ 
    nullmod <- fitNullModel(scanAnnot, outcome = "pheno", covars = Covars, cov.mat = myGRM, max.iter = 10000)
  }
